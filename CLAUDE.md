...
- MapLibre GL JS 5.6.1 from CDN (pinned + SRI, same version as `playground/`) — one hero globe (`#mapGlobe`, `projection: globe`) in a no-page-scroll ArcGIS-style shell (left origin dock, right antipode dock, bottom orthodrome console). Ported from Leaflet 2026-07-18, then consolidated from split origin/antipode panels into a single globe 2026-07-19, per the repo's `.claude/skills/globe-maps` skill. Zoom constants carried over from Leaflet get −1 via the `gz()` helper (256px vs 512px tile bases); `maxZoom` itself is a fresh 21 (not gz'd), letting the Esri source (maxzoom 16) overzoom for "zoom in really far." The ±85.05° lat clip in the ring pipeline is still required: MapLibre's globe is the Web Mercator tile pyramid wrapped onto a sphere.
